...
{
    public float maxRadiation = 600f;   // 圆心最大辐射强度 (μSv/h)
    public float radius = 30f;           // 辐射区域半径
    ...
    void Update()
    {
        ...
        if (player != null && player.cameraController != null && player.cameraController.IsIndoors())
        {
            ...
            {
                SFXManager.Instance.StopRadiationAlarm();
            }
        }
        else
        {
            ...
            if (SFXManager.Instance != null)
            {
                SFXManager.Instance.PlayRadiationAlarm(totalRadiation, maxRadiation);
            }
        }
    }
